...
def plot_jpg_labeling(img_file: str, labeling_txt: str, config: Union[str, Dict[str, Any]] = None) -> None:
    ...
    # Load configuration
    if config is None:
        # Load from package base.yml
        package_dir = os.path.dirname(os.path.abspath(__file__))
        config_file = os.path.join(package_dir, 'base.yml')
...
def export_yolo_label(bursts: List[List[float]], burst_type: List[bool], 
    ...
    Note:
        - Type 0: t3 (Type III radio burst without fine structure)
        - Type 1: t3b (Type III radio burst with fine structures)
